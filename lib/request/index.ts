...
class HttpClient {
  ...
  /** 调用 /auth/refresh（通过 X-Refresh-Token header 传递）*/
  ...
  private async handleResponse<T>(response: Response): Promise<ApiResponse<T>> {
    ...
    if (response.status === 401) {
      ...
      const error = new ApiError(data.message || 'Unauthorized', 401, data.data);
  ...
  async request<T>(
    ...
    const { method = 'GET', body, params, timeout, signal, headers, _isRetry = false } = config;
